...
- Counts: 61 base cmds / 11 categories + 23 CMS cmds. 24 Lua libs / 207 fns + 1 CMS lib (gnode_asset.lua) / 10 = **217** FCALL fns (COMMAND_SCHEMA.md).
...
- Tool-tier affordances: `entity.m.affordances` = [`scheme:target`] — `describe:<component>` (machine-readable cmd schemas) | `cli:<repo>/geodeploy.yaml` (the manifest declaring its commands; `geodineum --help` renders them) | `doc:<repo>/<path>` (floor). Returned verbatim by `service_describe`.m. Sibling of `schema_keys`, same pointer-to-artifact role. Declared in config/ecosystem_tools.yaml.
